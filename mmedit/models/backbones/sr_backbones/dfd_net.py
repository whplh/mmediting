    ...
    def __init__(self,
        ...
        modules = [
            spectral_norm(nn.Conv2d(
                in_channels, mid_channels, kernel_size, stride, padding,
                dilation=dilation, bias=bias)),
            nn.LeakyReLU(0.2),
            spectral_norm(nn.Conv2d(
                mid_channels, out_channels, kernel_size, stride, padding,
                dilation=dilation, bias=bias))]
...
class MSDilateBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_last = spectral_norm(nn.Conv2d(
...
class Blur(nn.Module):
    ...
    def __init__(self, channels):
        ...
        self.register_buffer('weight_flip', weight_flip.repeat(channels, 1, 1, 1))
...
class VGGFeat(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.register_parameter(
            "rgb_mean", nn.Parameter(
                torch.Tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)))
        self.register_parameter(
            "rgb_std", nn.Parameter(
...
class DFDNet(nn.Module):
    ...
    def __init__(self, mid_channels=64):
        ...
        #norm
        self.vgg_extract = VGGFeat()
        
        self.ms_dilate = MSDilateBlock(mid_channels*8, dilations=[4,3,2,1])
    ...
    def forward(self, input, locations, dictionary):
        ...
        def compare(a, b, name):
            a = a[name].cpu()
            b = b.cpu()
            print(name, 'load', a.shape, a.max(), a.min())
            print(name, 'this', b.shape, b.max(), b.min())
            print((a-b).max())
        load = torch.load('/mnt/lustre/liyinshuo/01-git-clone/DFDNet/user/example_tensor.pth')
        compare(load, input, 'input')

        vgg_features = self.vgg_extract(input)
        update_vgg_features = []
        # compare(load, vgg_features, 'vgg_features')
        compare(load, locations['left_eye'], 'part_locations0')
        compare(load, locations['right_eye'], 'part_locations1')
        compare(load, locations['nose'], 'part_locations2')
        compare(load, locations['mouth'], 'part_locations3')
        print(dictionary.keys())
        for i, feature_size in enumerate(self.feature_sizes):
            compare(load, vgg_features[i], f'vgg_features{i}')
            # vgg_features[i] = load[f'vgg_features{i}'].cpu()
            ...
            left_eye_location = (locations['left_eye'][0] // (512/feature_size)).int()
            right_eye_location = (locations['right_eye'][0] // (512/feature_size)).int()
            nose_location = (locations['nose'][0] // (512/feature_size)).int()
            mouth_location = (locations['mouth'][0] // (512/feature_size)).int()

            left_eye_feature = feature[:,:,left_eye_location[1]:left_eye_location[3],left_eye_location[0]:left_eye_location[2]].clone()
            right_eye_feature = feature[:,:,right_eye_location[1]:right_eye_location[3],right_eye_location[0]:right_eye_location[2]].clone()
            nose_feature = feature[:,:,nose_location[1]:nose_location[3],nose_location[0]:nose_location[2]].clone()
            mouth_feature = feature[:,:,mouth_location[1]:mouth_location[3],mouth_location[0]:mouth_location[2]].clone()
            
            #resize
            left_eye_feature_resize = F.interpolate(left_eye_feature,(left_eye_dict_feature.size(2),left_eye_dict_feature.size(3)),mode='bilinear',align_corners=False)
            right_eye_feature_resize = F.interpolate(right_eye_feature,(right_eye_dict_feature.size(2),right_eye_dict_feature.size(3)),mode='bilinear',align_corners=False)
            nose_feature_resize = F.interpolate(nose_feature,(nose_dict_feature.size(2),nose_dict_feature.size(3)),mode='bilinear',align_corners=False)
            mouth_feature_resize = F.interpolate(mouth_feature,(mouth_dict_feature.size(2),mouth_dict_feature.size(3)),mode='bilinear',align_corners=False)

            left_eye_Dict_feature_norm = adaptive_instance_normalization(left_eye_dict_feature, left_eye_feature_resize)
            right_eye_Dict_feature_norm = adaptive_instance_normalization(right_eye_dict_feature, right_eye_feature_resize)
            nose_Dict_feature_norm = adaptive_instance_normalization(nose_dict_feature, nose_feature_resize)
            mouth_Dict_feature_norm = adaptive_instance_normalization(mouth_dict_feature, mouth_feature_resize)
            
            left_eye_score = F.conv2d(left_eye_feature_resize, left_eye_Dict_feature_norm)

            left_eye_score = F.softmax(left_eye_score.view(-1),dim=0)
            left_eye_index = torch.argmax(left_eye_score)
            left_eye_swap_feature = F.interpolate(left_eye_Dict_feature_norm[left_eye_index:left_eye_index+1], (left_eye_feature.size(2), left_eye_feature.size(3)))

            left_eye_block = getattr(self, 'left_eye_'+str(feature_size))
            left_eye_attention = left_eye_block(left_eye_swap_feature-left_eye_feature)
            left_eye_attention_feature = left_eye_attention * left_eye_swap_feature
            
            right_eye_score = F.conv2d(right_eye_feature_resize, right_eye_Dict_feature_norm)
            right_eye_score = F.softmax(right_eye_score.view(-1),dim=0)
            right_eye_index = torch.argmax(right_eye_score)
            right_eye_swap_feature = F.interpolate(right_eye_Dict_feature_norm[right_eye_index:right_eye_index+1], (right_eye_feature.size(2), right_eye_feature.size(3)))
            
            right_eye_block = getattr(self, 'right_eye_'+str(feature_size))
            right_eye_attention = right_eye_block(right_eye_swap_feature-right_eye_feature)
            right_eye_attention_feature = right_eye_attention * right_eye_swap_feature

            nose_score = F.conv2d(nose_feature_resize, nose_Dict_feature_norm)
            nose_score = F.softmax(nose_score.view(-1),dim=0)
            nose_index = torch.argmax(nose_score)
            nose_swap_feature = F.interpolate(nose_Dict_feature_norm[nose_index:nose_index+1], (nose_feature.size(2), nose_feature.size(3)))
            
            nose_block = getattr(self, 'nose_'+str(feature_size))
            nose_attention = nose_block(nose_swap_feature-nose_feature)
            nose_attention_feature = nose_attention * nose_swap_feature

            mouth_score = F.conv2d(mouth_feature_resize, mouth_Dict_feature_norm)
            mouth_score = F.softmax(mouth_score.view(-1),dim=0)
            mouth_index = torch.argmax(mouth_score)
            mouth_swap_feature = F.interpolate(mouth_Dict_feature_norm[mouth_index:mouth_index+1], (mouth_feature.size(2), mouth_feature.size(3)))
            
            mouth_block = getattr(self, 'mouth_'+str(feature_size))
            mouth_attention = mouth_block(mouth_swap_feature-mouth_feature)
            mouth_attention_feature = mouth_attention * mouth_swap_feature

            update_feature[:,:,left_eye_location[1]:left_eye_location[3],left_eye_location[0]:left_eye_location[2]] = left_eye_attention_feature + left_eye_feature
            update_feature[:,:,right_eye_location[1]:right_eye_location[3],right_eye_location[0]:right_eye_location[2]] = right_eye_attention_feature + right_eye_feature
            update_feature[:,:,nose_location[1]:nose_location[3],nose_location[0]:nose_location[2]] = nose_attention_feature + nose_feature
            update_feature[:,:,mouth_location[1]:mouth_location[3],mouth_location[0]:mouth_location[2]] = mouth_attention_feature + mouth_feature
        ...
        compare(load, feature_vgg, 'fea_vgg')
        compare(load, feature_up0, 'fea_up0')
        compare(load, feature_up1, 'fea_up1')
        compare(load, feature_up2, 'fea_up2')
        compare(load, feature_up3, 'fea_up3')
        compare(load, output, 'output')
